...
export function findPath(obj, targetString) {
    for (const key in obj) {
        ...
        if (key.toLowerCase() === targetString.toLowerCase()) return path;

        if (typeof obj[key] === 'object') {
            for (const key2 in obj[key]) {
                path.grupi_2 = key2;
                if (key2.toLowerCase() === targetString.toLowerCase()) return path;
...
const key1 = 'nazva_harakteristiki_1';
const characteristic1 = 'znachennya_harakteristiki_1';

const key2 = 'nazva_harakteristiki_2';
const characteristic2 = 'znachennya_harakteristiki_2';

export function getCharacteristicsFilters(items = []) {
    return items.reduce((acc, item) => {
        if (key1 in item) acc = getCharacteristics(acc, item, key1, characteristic1);
        if (key2 in item) acc = getCharacteristics(acc, item, key2, characteristic2);
        return acc;
    }, {});
}
...
export function getButtonText(id) {
    ...
    return items.includes(id.toString()) ? BTN_TEXT_DELETE_PRODUCT : BTN_TEXT_ADD_PRODUCT;
}