...
function createList(items = [], name) {
    ...
    items.forEach((item, index) => {
        const toggle = names.includes(name?.toUpperCase()) && values.includes(item.toString());
        const hiddenClass = index < 5 ? 'expanded' : '';

        menu += `<li class='${hiddenClass}'>
                    <label>
                        <input type="checkbox" name="${name}" value="${item}" ${toggle ? 'checked' : ''}> ${item}
                    </label>
                 </li>`;
    })
    menu += items.length > 5 ? `<li class="show-more expanded">Показати більше</li>` : '';
    menu += '</ul>';
    return menu;
}
