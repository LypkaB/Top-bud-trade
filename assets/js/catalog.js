...
import {enscartEl, catalogGridEl, sortSelectEl, catalogSidebarEl, sidebarFilterEl} from './refs.js';
import {filteredData, updateMarkup, createADataObj, getMenuStructure, getCharacteristicsFilters, filterDataByCharacteristics,} from './helper/index.js';
import {getDataFromStorage, saveDataToSectionStore, getDataFromSectionStore,} from './api/sectionstore.js';
import {setToLocalStorage, getFromLocalStorage, addToLocalStorage} from './api/localstorage.js';
...
function checkCharacteristicsCheckbox() {
    const checkedCheckboxes = document.querySelectorAll('.filter__characteristics-list input[type="checkbox"]:checked');

    if (checkedCheckboxes.length > 0) {
        checkedCheckboxes.forEach(checkbox => {
            const checkboxParent = checkbox.closest('ul');
            const showMoreBtn = checkboxParent.querySelector('.show-more');

            checkboxParent.querySelectorAll('li').forEach(elem => elem.classList.add('expanded'));
